...
final class ImGuiAppDelegate : ApplicationDelegate {
    ...
    func applicationRenderedImGui(_ application: Application, frame: UInt64, renderData: ImGui.RenderData, window: Window, scissorRect: ScissorRect) async {
        ...
        let renderTargetsDescriptor = await RenderTargetsDescriptor(colorAttachments: [.init(texture: window.texture)])
        ...
        await application.windowRenderGraph.addClearPass(renderTargets: renderTargetsDescriptor, colorClearOperations: [.clear(ClearColor(red: clearRed, green: clearGreen, blue: clearBlue, alpha: 1.0))])
        
        await application.windowRenderGraph.addPass(ImGuiPass(renderData: renderData, renderTargetsDescriptor: renderTargetsDescriptor))
    }
}
